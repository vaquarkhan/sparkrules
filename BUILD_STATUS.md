...
| `pytest tests` (default, `-m "not perf"`) | **352 passed**, 2 skipped, 1 deselected (perf) |
...
Full target from `idea-brainstrom.txt` §8–9: **352 passed, 2 skipped, 1 deselected** (property P1–P38, integration, unit ladder) — **met**. Perf is excluded by default via `pyproject.toml` `addopts`.

### `idea-brainstrom.txt` §34.2 exit review

| Criterion | Status |
|-----------|--------|
| `pytest tests/ -q` → `352 passed, 2 skipped, 1 deselected` | **Met** (on this tree) |
