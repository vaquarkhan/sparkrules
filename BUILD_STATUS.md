...
| `pytest tests` (default, `-m "not perf"`) | **364 passed**, 2 skipped, 1 deselected (perf) |
...
**364 passed, 2 skipped, 1 deselected** on the default test selection. Perf is excluded by default via `pyproject.toml` `addopts`.

### `idea-brainstrom.txt` §34.2 exit review

| Criterion | Status |
|-----------|--------|
| `pytest tests/ -q` → `364 passed, 2 skipped, 1 deselected` | **Met** (on this tree) |
