...
| `pytest tests` (default, `-m "not perf"`) | **840+ passed**, 2 skipped, 1 deselected (perf) |
...
**840+ passed, 2 skipped, 1 deselected** on the default test selection. Perf is excluded by default via `pyproject.toml` `addopts`. The `src/sparkrules` 100% line gate is enforced on `tests/unit/` via `pytest tests/unit/ --cov=src/sparkrules` (or equivalent).

### `idea-brainstrom.txt` §34.2 exit review

| Criterion | Status |
|-----------|--------|
| `pytest tests/ -q` → `840+ passed, 2 skipped, 1 deselected` | **Met** (on this tree) |
