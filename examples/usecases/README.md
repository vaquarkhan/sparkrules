...
Each subdirectory is a **self-contained SparkRules demo**: sample **DRL**, **CSV** tuned for that storyline, **`validate_csv.py`** (no JVM — uses `iter_rule_rows`), **`spark_e2e.py`** (broadcast + `apply_drl` when PySpark/Java are available), and **`EXAMPLE.md`** with detailed rule explanations.
