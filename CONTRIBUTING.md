...
Line coverage for `sre` is enforced at **100%** in `pyproject.toml` when you run with `--cov=sre` (see `BUILD_STATUS.md`).
...
- Update `BUILD_STATUS.md` if the default test or coverage story changes in a way users must know.
