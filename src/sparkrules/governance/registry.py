...
ADJACENT_PROMOTIONS: frozenset[tuple[str, str]] = frozenset({("dev", "stage"), ("stage", "prod")})
...
class PromotionRegistry:
    ...
    def get_pin(self, namespace: str, handle: str, environment: str) -> int | None:
        return self._pins.get((namespace, handle, environment))

    def set_pin(self, namespace: str, handle: str, environment: str, version: int) -> None:
        if environment not in STANDARD_ENVS:
            raise ValueError(f"unknown environment: {environment!r}")
        self._pins[(namespace, handle, environment)] = version

    def promote(self, namespace: str, handle: str, from_env: str, to_env: str) -> int:
        if (from_env, to_env) not in ADJACENT_PROMOTIONS:
            raise ValueError("only adjacent promotion is supported (dev→stage, stage→prod)")
    ...
    def all_pins(self, namespace: str | None = None) -> list[dict[str, Any]]:
