# Documentation

**Full hosted docs: [sparkrules.readthedocs.io](https://sparkrules.readthedocs.io)**

## Start here

| Document | Purpose |
|----------|---------|
| [FEATURES.md](FEATURES.md) | Complete feature list: V2 engine, regulatory compliance, DQ, policy export |
| [HOW_IT_WORKS.md](HOW_IT_WORKS.md) | Architecture: parsing, compilation, alpha network, execution strategies |
| [SPARK_INTEGRATION.md](SPARK_INTEGRATION.md) | When to use Spark vs pure Python, `apply_drl(use_v2=True)` |
| [DEVELOPER_GUIDE.md](DEVELOPER_GUIDE.md) | Setup, run, test, and extension guidance |

## Use cases and examples

| Document | Purpose |
|----------|---------|
| [USE_CASES.md](USE_CASES.md) | Practical scenarios: lending, fraud, insurance, clinical, POS |
| [../examples/README.md](../examples/README.md) | 20+ runnable examples: DRL files, Python scripts, Spark jobs |
| [../examples/notebooks/](../examples/notebooks/README.md) | 6 Jupyter notebooks: tutorials + real-world use cases |

## Operations

| Document | Purpose |
|----------|---------|
| [GOVERNANCE.md](GOVERNANCE.md) | Namespaces, dev/stage/prod promotion, deprecation workflow |
| [KNOWN_LIMITATIONS.md](KNOWN_LIMITATIONS.md) | Architecture scope, extension points, deployment patterns |
| [BENCHMARKS.md](BENCHMARKS.md) | Performance methodology and measured throughput |
| [PUBLISHING.md](PUBLISHING.md) | PyPI, GitHub Container Registry, CI release process |
| [ROADMAP.md](ROADMAP.md) | Phases and future scope |
| [REQUIREMENTS_V2_ENGINE.md](REQUIREMENTS_V2_ENGINE.md) | **Normative V2 requirements** Req 1–37 (core + phased Spec A/B/C, non-goals, rollout, observability, security); implementation status summarized in-document |
| [../deploy/README.md](../deploy/README.md) | Cloud platform deployment: Glue, Databricks, Dataproc, Synapse, K8s |

## Quick links

- [Install guide](install.md)
- [Quick start tutorial](quickstart.md)
- [CHANGELOG](../CHANGELOG.md)
- [CONTRIBUTING](../CONTRIBUTING.md)
- [AGENTS.md](../AGENTS.md) (for AI coding agents)
- [SECURITY.md](../SECURITY.md)
