...
def test_detect_runtime_issues_for_slow_shuffle_and_failures() -> None:
    stages = [
        StageMetric(stage_id=1, duration_ms=130000, shuffle_read_mb=800, shuffle_write_mb=400, failed_tasks=2),
        StageMetric(stage_id=2, duration_ms=1000, shuffle_read_mb=1, shuffle_write_mb=1, failed_tasks=0),
    ]
...
def test_runtime_health_summary_and_ui_payload() -> None:
    stages = [StageMetric(stage_id=1, duration_ms=10, shuffle_read_mb=1, shuffle_write_mb=2, failed_tasks=0)]
...
def test_runtime_health_empty_and_warning_states() -> None:
    ...
    bad = summarize_runtime_health([
        StageMetric(stage_id=9, duration_ms=200000, shuffle_read_mb=2000, shuffle_write_mb=0, failed_tasks=0)
    ])
