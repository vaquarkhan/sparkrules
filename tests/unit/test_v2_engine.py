...
def test_translate_comparison_operators() -> None:
    for op_str, op_enum in [("==", "="), ("!=", "!="), ("<", "<"), ("<=", "<="), (">", ">"), (">=", ">=")]:
        expr = BinaryOp(getattr(BinaryOperator, {"==": "EQ", "!=": "NE", "<": "LT", "<=": "LE", ">": "GT", ">=": "GE"}[op_str]), Identifier("$t.x"), Literal(10))
...
def test_translate_in_non_list() -> None:
    expr = InExpr(Literal("x"), Identifier("$t.allowed"), negated=False)
    sql = translate_predicate(expr)
    assert "array_contains" in sql
...
def test_closure_comparison_operators() -> None:
    fact = {"t": {"x": 10}}
    for op, expected in [(BinaryOperator.GT, True), (BinaryOperator.LT, False), (BinaryOperator.EQ, False), (BinaryOperator.GE, True)]:
...
def test_closure_and_or() -> None:
    fact = {"t": {"x": 10, "y": 3}}
    and_expr = BinaryOp(BinaryOperator.AND, BinaryOp(BinaryOperator.GT, Identifier("$t.x"), Literal(5)), BinaryOp(BinaryOperator.LT, Identifier("$t.y"), Literal(5)))
    assert compile_predicate(and_expr)(fact) is True
    or_expr = BinaryOp(BinaryOperator.OR, BinaryOp(BinaryOperator.GT, Identifier("$t.x"), Literal(100)), BinaryOp(BinaryOperator.LT, Identifier("$t.y"), Literal(5)))
...
def test_flatten_and_chain() -> None:
    expr = BinaryOp(BinaryOperator.AND, BinaryOp(BinaryOperator.AND, Literal(1), Literal(2)), Literal(3))
...
def test_translate_in_negated_non_list() -> None:
    expr = InExpr(Literal("x"), Identifier("$t.arr"), negated=True)
    sql = translate_predicate(expr)
    assert "NOT" in sql
    assert "array_contains" in sql
